...
public class OrderBiz {
    ...
    //根据日期获得订单列表
    public void selectOrdersByDate(final String date, final Handler handler) {
        new Thread() {
            @Override
            public void run() {
                String url = Const.URL_SHOP_ORDERS;
                StringBuilder builder = new StringBuilder(url);
                builder.append(TApplication.shop.getShopId()).append("&createDate=").append(date);
                Message message = handler.obtainMessage();
                url = builder.toString();
                String jsonString = HttpUtils.doGetAsyn(url);
                boolean status = JsonUtils.getStatus(jsonString);
                if(status){
                    TApplication.orderAmount = JsonUtils.getAmount(jsonString);
                    TApplication.orderArrays.append(10,JsonUtils.getShopOrders(jsonString));
    ...
    public void insertRefuse(final String reason, final Order order, final Handler handler) {
        new Thread() {
            ...
            public void run() {
                ...
                boolean status = JsonUtils.getStatus(jsonString);
                message.obj = JsonUtils.getDescription(jsonString);
                if(status){
                    //取消订单列表中新增，新订单列表中移除
    ...
    public void insertReiveOrder(final Order order, final Handler handler) {
        new Thread() {
            ...
            public void run() {
                ...
                message.obj = desc;
                if(status){//接单成功后 已接单列表中新增，新订单移除
